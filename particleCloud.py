...
class particleCloud:
    ...
    def updateStraight(self, p, dX, dY):
        ...
        X = Z * (1 + np.random.normal(0, 0.05)) # 5% error
    ...
    def update_sonar(self):
        t1 = time.clock()
        sonarMeasurement, sonarAngle = self.sonar.getSonarAndAngleReadingNoTurn() 
        t2 = time.clock()
    ...
    def _likelihoodCloud_rt(self, sonarMeasurement, sonarAngle):
        ...
        for i in range(self.numberOfParticles):
            ...
            if minDistance < np.inf:
                ...
                # likelihood = math.exp(exponent) + 0.25 #0.02 # + 1 / float(self.numberOfParticles)
                likelihood = math.exp(exponent) + 0.01 #0.02 # + 1 / float(self.numberOfParticles)
